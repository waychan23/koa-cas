 ...
 * @returns {*}
 */
module.exports = async function (ctx, proxyOptions) {
  ...
  const isMatchFilter = (options.cache && options.cache.filter && typeof options.cache.filter.some === 'function') ? options.cache.filter.some(function(rule) {
    return utils.isMatchRule(ctx, targetService, rule);
  }) : false;
  ...
  if (disableCache || !options.cache.enable || isMatchFilter || renew) {
    ...
    try {
      const pt = await requestPT(proxyPath, retryHandler);
      if (renew) {
        logger.info(`Refetch a new pt succeed, pt: ${pt}. Try store it in cache.`);
        await ptStore.set(ctx, targetService, pt);
      }
      return pt;
    } catch (err) {
      ...
      if (err.message.indexOf('Request for PT succeed, but the response is invalid') !== -1) {
        ...
        return ctx.redirect(utils.getPath('login', options));
      }
      logger.error(err);
      throw err;
    }
  ...
  // Use cache
  try {
    let pt = await ptStore.get(ctx, targetService);
    if (pt) {
    ...
    try {
      pt = await requestPT(proxyPath, retryHandler);
   ...
   */
  async function requestPT(path, retryHandler) {
    logger.info('Trying to request proxy ticket from ', proxyPath);
    const startTime = Date.now();
    try {
      const response = await utils.getRequest(path);
      ...
      if (pt) {
        ...
        if (typeof retryHandler === 'function') {
          return await retryHandler(new Error(`Request for PT succeed, but response is invalid, response: ${response.body}`));
        }
      ...
      logger.error(err);
      if (typeof retryHandler === 'function') return await retryHandler(err);
      throw err;
    }
  }
};
