...
async function getProxyTicketThroughRestletReq(ctx, targetService, restletOptions) {
  ...
  if (!isUsingCache) logger.info('Don\'t use cached pgt by options.restletIntegrationIsUsingCache config.');
  let pgt = isUsingCache ? globalPGTStore.get(restletIntegrateRuleKey) : null;

  async function retryHandler(err) {
    if (doNotRetry === true) {
    ...
    return await getProxyTicketThroughRestletReq.call(that, ctx, targetService, {
  ...
  if (pgt) {
    ...
    return await getProxyTicket.call(that, ctx, {
      targetService,
      specialPgt: pgt,
      disableCache: true,
      retryHandler,
  ...
  let response = null;
  try {
    response = await utils.postRequest(path, restletParams);
  ...
  if (isUsingCache) globalPGTStore.set(restletIntegrateRuleKey, pgt);
