 ...
 * @param options
 */
async function validateTicket(ctx, options) {
  ...
  try {
    const response = await utils.getRequest(serviceValidateUrl);
 ...
 * @param options
 */
async function retrievePGTFromPGTIOU(ctx, afterHook, pgtIou, options) {
  const logger = utils.getLogger(ctx, options);
  logger.info('Trying to retrieve pgtId from pgtIou...');
  try {
    const session = await ctx.sessionStore.get(pgtIou);
    if (session && session.pgtId) {
      ...
      try {
        // 释放
        await ctx.sessionStore.destroy(pgtIou);
 ...
 * @param options
 */
module.exports = async function validate(ctx, afterHook, options) {
  // check ticket first
  ...
  try {
    ...
    try {
      const info = await parseCasResponse(response.body, logger);
      ...
      if (options.slo) {
        try {
          await ctx.sessionStore.set(ticket, {
            sid: ctx.sessionId,
      ...
      if (pgtIou) {
        return await retrievePGTFromPGTIOU(ctx, afterHook, pgtIou, options);
