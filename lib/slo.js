 ...
 * @param options
 */
module.exports = async function (ctx, afterHook, options) {
  ...
  try {
    ...
    const result = await ctx.sessionStore.get(st);
      /* istanbul ignore else */
    if (result && result.sid) {
      logger.info('Find sid by st succeed, trying to destroy it.', 'st: ', st, 'sessionId: ', result.sid);
      try {
        await ctx.sessionStore.destroy(result.sid);
        await ctx.sessionStore.destroy(st);
        await afterHook();
        ctx.status = 200;
        return;
      } catch (err) {
